...
# retrieve data store to MongoDB database
...
def mongodb(database):
    ...
    col = db.list_collection_names()
    if len(col) == 0:
        st.info("There is no data Retrieved from youtube")
    else:
        data_youtube = {}
        amri = pymongo.MongoClient(
            "mongodb://127.0.0.1:27017/")
...
def sql_create_tables():
    amri_sql = psycopg2.connect(host='localhost', user='postgres', password='12345', database='youtube')
    cursor = amri_sql.cursor()
...
def list_sql_channel_names():
    amri = psycopg2.connect(host='localhost', user='postgres', password='12345', database='youtube')
    cursor = amri.cursor()
...
def sql_channel(database, col_input):
    amri_mdb = pymongo.MongoClient(
        "mongodb://127.0.0.1:27017/")
    db = amri_mdb[database]
    col = db[col_input]
...
def channels_totalchannels():
    ...
    amri_s = psycopg2.connect(host='localhost', user='postgres', password='12345', database='youtube')
    cursor = amri_s.cursor()
...
def sql_queries():
    ...
    q5 = 'Q5-Which videos have the highest number of likes, and what are their corresponding channel names?'
    q6 = 'Q6-What is the total number of likes for each video with their corresponding video names?'
    q7 = 'Q7-What is the total number of views for each channel, and what are their corresponding channel names?'
    ...
    try:
        if option == 'Retrieving data from the YouTube API':
            ...
            if submit_c:
                ...
                st.success('Retrieved data from YouTube successfully')
                st.balloons()
